...
def web_crawl(file_name):
    ...
    process.crawl(MySpider, domain=domain, start_urls=urls, n=n)
    process.start()
...
class MySpider(CrawlSpider):
    ...
    def __init__(self, domain=None, start_urls=None, n=10, *args, **kwargs):
        super(MySpider, self).__init__(*args, **kwargs)
        if domain:
            self.allowed_domains = [domain]
        if start_urls:
            self.start_urls = start_urls[:n]
    ...
    def parse_item(self, response):
        ...
        to_url = response.url
        if from_url and to_url not in self.graph[from_url]:
            self.graph[from_url].append(to_url)
